...
async def invite_user(
    ...
    initial_password = data.password or "temporal123!"
    ...
    if settings.require_email_verification:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail="Invitation creee mais l'email n'a pas pu etre envoye.",
            )
        return {
    ...
    return {
        "message": f"Compte cree pour {data.email} avec le role {role.value}. Communiquez le mot de passe au membre.",
        "email": data.email,
    }
